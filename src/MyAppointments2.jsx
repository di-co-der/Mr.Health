  ...
  return (
    <div className="max-w-md mx-auto p-4">
      <div className="flex justify-around mb-4">
        <button
          className={`py-2 px-4 ${selectedTab === 'missed' ? 'border-b-2 border-blue-500' : ''}`}
          onClick={() => setSelectedTab('missed')}
        >
          Missed
        </button>
        <button
          className={`py-2 px-4 ${selectedTab === 'upcoming' ? 'border-b-2 border-blue-500' : ''}`}
          onClick={() => setSelectedTab('upcoming')}
        >
          Upcoming
        </button>
        <button
          className={`py-2 px-4 ${selectedTab === 'completed' ? 'border-b-2 border-blue-500' : ''}`}
      ...
      <div>
              ...
              <div key={index} className="border rounded-lg p-4 mb-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xl font-bold">{appointment.doctor}</div>
        ...
        {/* Similar blocks for "missed" and "completed" tabs */}
      </div>
    </div>
  );
};
